...
function docFalso() {
  const criar = (tag) => ({
    ...
    title: '', type: '', disabled: false, onclick: null, oninput: null, onblur: null,
    ...
    get campos() { return this.filhos.flatMap((f) => (f.tag === 'input' || f.tag === 'textarea' ? [f] : f.campos)) },
  })
...
test('a trilha diz onde se esta', () => {
  assert.match(montar({ passo: 0 }).corpo.texto, /passo 1 de 4/)
  assert.match(montar({ passo: 2, estado: cheio() }).corpo.texto, /passo 3 de 4/)
})

test('passo fora da faixa nao estoura — encaixa no limite', () => {
  assert.match(montar({ passo: 99, estado: cheio() }).corpo.texto, /passo 4 de 4/)
  assert.match(montar({ passo: -5 }).corpo.texto, /passo 1 de 4/)
})
...
test('o passo 3 resume o publico escolhido, e diz quando nao ha', () => {
  assert.match(montar({ passo: 2, estado: cheio() }).corpo.texto, /Onde: Campinas/)
  assert.match(montar({ passo: 2 }).corpo.texto, /Ainda não escolhido/)
})

test('o passo 4 marca a imagem escolhida e oferece enviar', () => {
  const { corpo } = montar({ passo: 3, estado: { ...cheio(), imagemHash: 'h2' }, aoEnviarImagem() {} })
...
test('sem callback de envio, o botao de enviar nao aparece', () => {
  ...
  assert.ok(!/enviar imagem/.test(montar({ passo: 3, estado: cheio() }).corpo.texto))
})
...
test('o primeiro passo nao tem Voltar; o ultimo troca Avancar por Criar', () => {
  ...
  const fim = montar({ passo: 3, estado: cheio() })
...
test('Criar com algo faltando LEVA ate o passo que falta, em vez de recusar', () => {
  let levou = null
  const { rodape } = montar({ passo: 3, estado: { ...cheio(), nome: '' }, aoIrPara: (p) => { levou = p }, aoCriar() { throw new Error('não podia criar') } })
...
test('enquanto cria, o botao trava e avisa', () => {
  const { rodape } = montar({ passo: 3, estado: cheio(), criando: true })
...
test('o rodape diz SEMPRE que nasce pausado', () => {
  for (const p of [0, 1, 2, 3]) assert.match(montar({ passo: p, estado: cheio() }).rodape.texto, /nasce pausado/)
})
